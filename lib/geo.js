...
/**
 * Every geolocation header the platform actually sent, for the health check.
 * Open /api/health on a phone on cellular to see what real visitor traffic
 * gets — some of these headers are only populated on paid Vercel plans, and
 * this is the quickest way to tell whether that's the problem.
 */
export function geoDebug(req) {
  const seen = {};
  for (const [name, value] of Object.entries(req.headers)) {
    if (name.toLowerCase().startsWith("x-vercel-ip-")) seen[name] = value;
  }
  return {
    headersPresent: Object.keys(seen),
