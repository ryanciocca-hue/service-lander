...
/**
 * The Neon and Vercel Postgres integrations inject the connection string under
 * different names depending on how the database was attached, so accept any of
 * them. Pooled URLs come first — serverless needs pgbouncer in front.
 */
...
export function connectionVarName() {
  return CONNECTION_VARS.find((name) => Boolean(process.env[name])) ?? null;
}

function getPool() {
  if (pool) return pool;

  const name = connectionVarName();
  if (!name) {
    throw new Error(`No Postgres connection string found (looked for ${CONNECTION_VARS.join(", ")})`);
  }
...
-- Added after launch; ALTER ... IF NOT EXISTS keeps cold-start setup idempotent.
ALTER TABLE submissions ADD COLUMN IF NOT EXISTS product TEXT;
